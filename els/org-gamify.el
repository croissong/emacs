...
(defcustom org-gamify-use-notify-p nil
  "If non-nil, use the function `notify' (http://www.emacswiki.org/emacs/notify.el)
instead of `message' to alert the user to currency changes. Notify
...
changed. Returns the message as a string."
  (let* ((currency-plist (org-gamify-get-currency-plist currency))
    ...
    (cond
     ((and org-gamify-use-notify-p
           (featurep 'notify))
      (notify "Currency changed" msg
...
is too far in the past, return 0."
  (let ((today (time-to-days (current-time)))
    ...
    (cond
     ((org-is-habit-p)
      (destructuring-bind (sched-date habit-min dead-date habit-max done-dates)
  ...
  (let ((buf (get-buffer-create "*Inventory*")))
    (with-current-buffer buf
      ...
      (dolist (category
        ...
        (when category
          (insert "\n")
          (insert (propertize category 'font-lock-face '(:weight bold :foreground "gold")))
          (insert "\n\n"))
